...
const Lk = () => {
  ...
  const [notifications, setNotifications] = useState<string[]>(['Обновление графика смен', 'Новое поручение от руководителя']);
  ...
  return (
    <div className={styles.container}>
        <div className="min-h-screen bg-black bg-opacity-100 bg-opacity-40 p-8">
      {/* Информация о пользователе */}
      <div className="max-w-6xl mx-auto bg-white bg-opacity-80 p-8 rounded-lg shadow-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white bg-opacity-80 backdrop-blur-lg rounded-lg shadow-md p-6">
        ...
        {/* Уведомления */}
        <div className="mt-8">
          <h3 className="text-2xl font-semibold text-red-600">Уведомления</h3>
          <ul className="space-y-4 mt-4">
            {notifications.map((note, index) => (
              <li key={index} className="p-4 bg-white rounded-lg shadow-sm hover:shadow-lg transition-all duration-300">
                {note}
              </li>
            ))}
          </ul>
